...
class MonitorStream extends OnEventTargetFactory(['progress', 'abort']).mixin(TransformStream) {
    constructor({ onprogress = null, onabort = null, throttle = 0, loaded = 0, total = Infinity, lengthComputable = false, progressInterval = 1000, } = {}, ...strategies) {
        let controller = null;
        let progressLast = 0;
        let last = 0;
        super({
            ...
            transform: throttle ?
                async (chunk, controller) => {
                    ...
                    last = now;
                    if (drift > 0)
                        await new Promise(resolve => setTimeout(resolve, 2 * drift));
...
//# sourceMappingURL=monitor-stream.js.map