...
class StreamDetailedFetchBlob extends BaseDetailedFetchBlob {
    constructor(input, { onprogress = null, onabort = null, onerror = null, loaded = 0, total = Infinity, lengthComputable = false, fetch = top.fetch, ...init } = {}) {
        super(input, { onprogress, onabort, onerror, loaded, total, lengthComputable });
        this.controller = new AbortController();
        this.promise = (async () => {
            try {
                const { body, ok, status, statusText, headers } = await fetch(input, { ...init, signal: this.controller.signal });
                if (!ok)
                    throw new DOMException(`${status}: ${statusText}`, 'HTTPError');
                if (!body)
                    throw new DOMException('DetailedFetchBlob encountered a network error', 'NetworkError');
...
//# sourceMappingURL=stream-detailed-fetch-blob.js.map