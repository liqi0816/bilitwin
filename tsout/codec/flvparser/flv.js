...
class FLV {
    constructor(dataView) {
        ...
        if (offset !== dataView.byteLength)
            throw new Error(`FLV construtor: final offset should equal dataView.byteLength but get offset ${offset} !== byteLength ${dataView.byteLength}`);
    }
    get type() {
        return 'FLV';
    }
    ...
    static merge(flvs) {
        if (!flvs[Symbol.iterator])
            throw new TypeError(`FLV.merge: parameter flvs expect Iterable<FLV> but get ${flvs}`);
        let blobParts = [];
    ...
    static async mergeBlobs(blobs) {
        ...
        if (!blobs[Symbol.iterator])
            throw new TypeError(`FLV.mergeBlobs: parameter blobs expect Iterable<Blob> but get ${blobs}`);
        let ret = [];
        let basetimestamp = [0, 0];
        let lasttimestamp = [0, 0];
        let duration = 0.0;
